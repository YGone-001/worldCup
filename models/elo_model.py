...
def enhanced_elo(base_elo: float, squad_value_million: float,
    ...
    if recent_form:
        ...
        # 引入基石难度系数：基础Elo越高，含金量越大
        form_factor = (weighted_form - 0.5) * 60 * (base_elo / 1600.0)
    else:
        form_factor = 0
...
def injury_penalty(team_data: dict) -> float:
    ...
    for inj in injuries:
        ...
        # 基础惩罚：核心球员扣除总 Elo 的 3.5%，普通球员扣除 1.0%
        is_key = any(kp in name or name in kp for kp in key_players)
        base_penalty = base_elo * 0.035 if is_key else base_elo * 0.010
